const Task = require('../models/Task');
const ObjectModel = require('../models/Object');
const { distanceInMeters } = require('../services/geoService');

const MAX_DIST_METERS = 1000000;

async function getProductsForObject(req, res) {
  try {
    const { taskId, objectId } = req.params;
    const { lat, lng } = req.query;

    console.log('getProductsForObject', { taskId, objectId, lat, lng });

    if (!lat || !lng) {
      return res.status(400).json({ message: 'lat и lng обязательны' });
    }

    const task = await Task.findById(taskId)
      .populate('objects.products.productId');

    if (!task) {
      console.error('Task not found', taskId);
      return res.status(404).json({ message: 'Task not found' });
    }

    const objectPart = task.objects.find(
      (o) => o.objectId.toString() === objectId
    );
    if (!objectPart) {
      return res.status(404).json({ message: 'Object in task not found' });
    }

    const object = await ObjectModel.findById(objectId);
    if (!object) {
      return res.status(404).json({ message: 'Object not found' });
    }

    if (
      !object.coords ||
      typeof object.coords.lat !== 'number' ||
      typeof object.coords.lng !== 'number'
    ) {
      console.error('Object has no coords', object);
      return res
        .status(400)
        .json({ message: 'У объекта не заданы координаты coords.lat/lng' });
    }

    const dist = distanceInMeters(
      Number(lat),
      Number(lng),
      object.coords.lat,
      object.coords.lng
    );

    console.log('distance =', dist);

    if (dist > MAX_DIST_METERS) {
      return res.status(403).json({
        message: 'Геолокация не совпадает с объектом',
        distance: dist,
      });
    }

    const products = objectPart.products.map((p) => {
      const prod = p.productId && p.productId._id ? p.productId : null;

      const photos = p.photos || [];
      const lastPhoto = photos.length > 0 ? photos[photos.length - 1] : null;

      return {
        _id: prod?._id ?? p.productId,
        productId: prod?._id ?? p.productId,

        name: prod?.name,
        category: prod?.category,
        imageUrl: prod?.imageUrl,
        iconUrl: prod?.iconUrl,

        status: p.status,
        price: p.price,
        photoUrl: lastPhoto ? lastPhoto.url : null,

        photos,
      };
    });

    return res.json({
      objectId,
      distance: dist,
      products,
    });
  } catch (e) {
    console.error('getProductsForObject error:', e);
    return res.status(500).json({ message: 'Server error' });
  }
}

async function saveProductPhotoAndPrice(req, res) {
  try {
    const { taskId, objectId, productId } = req.params;
    const { price, lat, lng, debugIgnoreGeo } = req.body;

    const ignoreGeo = debugIgnoreGeo === '1' || debugIgnoreGeo === true;

    const files = Array.isArray(req.files) && req.files.length
      ? req.files
      : (req.file ? [req.file] : []);

    if (!files.length) {
      return res.status(400).json({ message: 'photo is required' });
    }
    if (!price) {
      return res.status(400).json({ message: 'price is required' });
    }
    if (!lat || !lng) {
      return res.status(400).json({ message: 'lat и lng обязательны' });
    }

    const task = await Task.findById(taskId);
    if (!task) return res.status(404).json({ message: 'Task not found' });

    const objectPart = task.objects.find(
      o => o.objectId.toString() === objectId
    );
    if (!objectPart) {
      return res.status(404).json({ message: 'Object in task not found' });
    }

    const object = await ObjectModel.findById(objectId);
    if (!object) {
      return res.status(404).json({ message: 'Object not found' });
    }

    const dist = distanceInMeters(
      Number(lat),
      Number(lng),
      object.coords.lat,
      object.coords.lng
    );

    if (!ignoreGeo && dist > MAX_DIST_METERS) {
      return res.status(403).json({
        message: 'Геолокация не совпадает с объектом',
        distance: dist,
      });
    }

    const product = objectPart.products.find(
      p => p.productId.toString() === productId
    );
    if (!product) {
      return res.status(404).json({ message: 'Product in task not found' });
    }

    files.forEach(file => {
      const photoUrl = `/uploads/${file.filename}`;
      product.photos.push({ url: photoUrl });
    });

    product.price = Number(price);
    product.status = 'added';

    await task.save();

    return res.json({
      message: 'Saved',
      product,
    });
  } catch (e) {
    console.error(e);
    return res.status(500).json({ message: 'Server error' });
  }
}


async function startTask(req, res) {
  try {
    const { taskId } = req.params;
    const { lat, lng } = req.body;

    const task = await Task.findById(taskId);
    if (!task) return res.status(404).json({ message: 'Task not found' });

    task.status = 'in_progress';
    task.startedAt = new Date();
    if (lat && lng) {
      task.startLat = Number(lat);
      task.startLng = Number(lng);
    }

    await task.save();
    return res.json(task);
  } catch (e) {
    console.error(e);
    return res.status(500).json({ message: 'Server error' });
  }
}

async function completeTask(req, res) {
  try {
    const { taskId } = req.params;
    const { lat, lng, force } = req.body;

    const task = await Task.findById(taskId)
      .populate('objects.products.productId');

    if (!task) return res.status(404).json({ message: 'Task not found' });

    const missing = [];

    task.objects.forEach(obj => {
      obj.products.forEach(p => {
        if (
          p.status !== 'added' ||
          !p.photos ||
          p.photos.length === 0 ||
          p.price == null
        ) {
          missing.push({
            productId: p.productId._id,
            name: p.productId.name,
          });
        }
      });
    });
    if (missing.length > 0 && !force) {
      return res.status(400).json({
        message: 'Не все продукты заполнены',
        missing,
      });
    }

    if (missing.length === 0) {
      task.status = 'completedNaturally';
    }
    
    if (missing.length > 0 && force) {
      task.status = 'completedForcefully';
    }

    task.finishedAt = new Date();
    
    if (lat && lng) {
      task.endLat = Number(lat);
      task.endLng = Number(lng);
    }

    await task.save();

    return res.json({
      message: 'Task completed',
      task,
      missing,
    });
  } catch (e) {
    console.error(e);
    return res.status(500).json({ message: 'Server error' });
  }
}

module.exports = {
  getProductsForObject,
  saveProductPhotoAndPrice,
  startTask,
  completeTask,
};